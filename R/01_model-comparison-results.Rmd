...
icer_mar <- markov_comb(params, parallel = TRUE) 
icer_mar_corr <- markov_comb_corr(params)
params$horizon = 10
icer_mar10_1y_corr <- markov_comb_corr(params)
params$interval = 365
icer_mar10_1d_corr <- markov_comb_corr(params) 


Sys.setenv(R_MAX_VSIZE = 16e9)
icer_mar1d <- markov_comb(params) 
params$interval = 100
icer_mar1y <- markov_comb(params) 

icer_mar1d_corr <- markov_comb_corr(params)
...
base_case_result <- 
  icer_deq %>% 
    ...
    mutate(model = c("DEQ","MRKCHT_UNADJ","MRKCHT_EMB","DES","MICRO")) %>% 
...
table_main_results <- 
  ...
  mutate(model = factor(model, levels = c("DEQ","DES","MRKCHT_UNADJ","MRKCHT_EMB","MICRO"), 
                        labels = c("Differential Equations","Discrete Event Simulation","Markov Cohort - Unadjusted","Markov Cohort - Embedded","Individual Microsimulation"))) %>% 
