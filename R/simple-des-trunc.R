...
tdes_simulation <- function(params, N = NULL)
{
  ...
  with(params, {
    x <- data.frame(
      ...
      secular_death = trunc(rgompertz(n, shape, rate))*365,
      indication    = trunc(rexp(n, r_a / 365) / 365)*365
    )
...
tdes_summary <- function(df, params,N = NULL)
{
  if (!is.null(N)) params$n = N
  with(c(df, params), {
    # Testing costs
...
tdes_icer <- function(params, reference=NULL, genotype=NULL, N = NULL, seed=123)
{
  set.seed(seed)
  params$p_o <- 0.0 # No testing, reference
  reference <- tdes_summary(if(is.null(reference)) tdes_simulation(params,N=N) else reference, params,N=N)
  
  set.seed(seed)
  params$p_o <- 1.0 # Genotype testing upon indication
  genotype   <- tdes_summary(if(is.null(genotype)) tdes_simulation(params,N=N) else genotype, params,N=N)
