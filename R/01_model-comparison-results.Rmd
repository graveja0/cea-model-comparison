...
names(icer_mar) = names(icer_deq)
source(here("R/simple-markov-corrected.R"))
icer_mar_cor <- markov_comb(params) 
...
table_main_results[,-1] %>% 
  ...
  tab_row_group(rows = 1:4, group = "Mean QALYs") %>% 
  tab_row_group(rows = 5:8, group = "Mean Costs") %>% 
  tab_row_group(rows = 9:12, group = "ICER") %>% 
  tab_row_group(rows = 13:16, group = "Net Monetary Benefit") %>% 
