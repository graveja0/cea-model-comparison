...
```{r}
des_sample_sizes <- c(1e6,2e6,3e6,5e6,6e6,7e6,8e6,9e6,1e7,2e7)
set.seed(123)
icer_des_N <- 
  des_sample_sizes %>% 
  future_map(~(des_icer(params,N=.x) %>% t() %>% data.frame() %>% mutate(N = .x)))  %>% 
  bind_rows()
...
icer_des_N %>% 
  ...
  scale_x_continuous(breaks = des_sample_sizes, labels = des_sample_sizes/1000000) + 
  annotate("text",y=icer_deq[["NMB"]],x=0,label = "DEQ: NMB",hjust=0) +
  annotate("text",y=icer_mar[["NMB"]],x=0,label = "Markov: NMB",hjust=0,colour="red")
