...
source(here("R/new_markov.R")) # Faster version of the Markov model. 
...
kable(tbl) %>% 
  as_image(width = 6, file = "figs/01_model-parameters.png")
#html5(tbl,fragment = TRUE) %>% 
  #as_image(width = 6, file = "figs/01_model-parameters.png", density = 300)
...
icer_mar2 <- markov_comb(params)
icer_deq; icer_deq[1] - icer_mar2[1]
icer_des <- des_icer(params,N = 1e7)
icer_deq[1] - icer_des[1]
icer_mar <- markov_icer(params)
icer_microsim <- microsim_icer(params)
...
# icer_deq = icer_results$DEQ
# icer_des = icer_results$DES
# icer_mar = icer_results$MAR
# icer_micro = icer_results$MICRO
...
base_case_result <- 
  icer_deq %>% 
    ...
    mutate(model = c("DEQ","MAR","DES","MICRO")) %>% 
...
table_main_results <- 
  ...
  mutate(model = factor(model, levels = c("DEQ","DES","MAR","MICRO"), 
                        labels = c("Differential Equations","Discrete Event Simulation","Markov Cohort","Individual Microsimulation"))) %>% 
...
table_main_results %>% 
  html5(fragment=TRUE) %>% 
  as_image(width = 6, file = "figs/01_main-results.png")
...
icer_microsim_N <-
 des_sample_sizes %>%
 future_map(~(microsim_icer(params,seed=123,N=.x) %>% t() %>% data.frame() %>% mutate(N = .x)))  %>%
 bind_rows()
icer_microsim_N %>% write_rds("output/icer_microsim_N.rds")
...
icer_des_N %>% 
  ...
  ylim(c(-300,300)) + 
  geom_point(data = icer_microsim_N ,pch=2) 
  #ylim(c(-1))
...
draws <- halton(n = 100, dim = length(params_psa_realistic))
